# ===----------------------------------------------------------------------=== #
# mojoBLAS: Symmetric Matrix-Vector Operations
...
# ===----------------------------------------------------------------------=== #
"""
Symmetric Matrix-Vector Operations (mojoblas.level2.symv).
==========================================================
            ...
            for j in range(n):
                if x[unsafe_offset=j] != 0:
                    ...
                    def fused_upper[
                        ...
                        temp2 += (
                            av * x.unsafe_load[width=width](i)
                        ).reduce_add()

                    vectorize[simd_width](j, fused_upper)
                    y[unsafe_offset=j] = (
                        y[unsafe_offset=j]
                        + temp1 * a[unsafe_offset=j + j * lda]
                        + alpha * temp2
                    )
        elif incx == 1:
            for j in range(n):
            ...
            for j in range(n):
                if x[unsafe_offset=j] != 0:
                    ...
                    def fused_lower[
                        ...
                        temp2 += (
                            av * x.unsafe_load[width=width](ii)
                        ).reduce_add()
